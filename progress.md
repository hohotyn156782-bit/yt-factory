...
- [x] Верификатор №2 (свежий контекст, адверсариальный): **CONFIRMED по всем 6 фиксам**. Доказательства: реальный build_video с форсом → meta.json qa.ok/forced=true на диске + mp4 в publish/ (совпадает с CI-глобом); exception в синке не роняет публикацию; SRT-фазз 2000 кейсов = 0 перекрытий (на старом коде симптом воспроизвёлся — 24 блока); era-фолбэк по 4 сценариям; реюз-кадры в CREDITS не попадают (лицензии чистые); секретов в git нет.
- [x] GitHub: репо **hohotyn156782-bit/yt-factory (private)** создан, master запушен (b495f19 фиксы + e4b5c44 доки + TRUTH RULE в _three_hooks/_polish). Public-флип и секрет SECRETS_ENV заблокированы permission-гейтом — команды владельцу в docs/SETUP_OWNER.md Часть 0, файл секретов: ~/.config/yt-factory/SECRETS_ENV_github.txt (600).
- [x] По замечаниям верификатора №2: TRUTH RULE добавлен и в промпты перегенерации хука (_three_hooks/_polish) — закрыт канал повторного выдумывания фактов.

## Принятые осознанно (не баги)
- mode="stock" (шортсы): era-запросы теперь идут в archive.org ПЕРЕД Pexels (побочный эффект era-фикса; анти-анахронизм важен и в шортсах, PD-плёнка в вертикальном кропе — норм. документальная эстетика).
- Форс-паблиш при сбое копии в publish/: TG-карточка уйдёт, CI-артефакт без mp4, ошибка в логе — мягкая деградация.
- Реюз era-кадра может повторяться в нескольких слотах при выжженной квоте — визуальная монотонность, не ошибка (панорамы разнонаправленные).

- [x] **2026-07-13: владелец дал добро — Часть 0 выполнена мной.** Репо public (gh repo view: PUBLIC), секрет SECRETS_ENV залит, CI-ран 29213187516 **success за 10 мин**: попытка 1 забракована vision-QA (FLUX-опечатка «THEEE EMPEROR») → пересборка → «Viking trade monopoly» QA ok, virality 84 → видеофайл ушёл в TG (964216249), артефакт videos-shorts-29213187516 (24.5МБ) выгружен, коммит-бэк state/ прошёл (8ddd43c).
- [x] Карточка длинной документалки Рокфеллера отправлена в TG владельцу (sent: True, файл 238МБ > лимита бота — в карточке локальный путь на D:).
- [x] Грабля GitHub: воркфлоу из репо-создающего пуша не индексируется — лечится коммитом, меняющим САМ файл воркфлоу (пустой коммит не помогает; actionlint exit 0, файл был валиден).

## В работе сейчас
- [ ] Владелец: проверить 2 видео в TG (short «Viking trade monopoly» + карточка Рокфеллера), затем Части 1-3 SETUP_OWNER.md (аккаунт/OAuth/аудит-форма)
- [ ] После проверки видео: раскомментировать cron в autopilot.yml (long Пн/Ср/Пт 09:00 UTC, shorts ежедневно 15:00 UTC)
